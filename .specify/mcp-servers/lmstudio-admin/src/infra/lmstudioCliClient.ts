...
import type { LmStudioCliConfig } from '../config/types.js';
import type {
  LmStudioHttpModel,
...
export interface LmStudioCliClient {
...
function runCli(config: LmStudioCliConfig, logger: Logger, args: string[]): Promise<{ stdout: string; stderr: string }> {
...
export function createLmStudioCliClient(config: LmStudioCliConfig, logger: Logger): LmStudioCliClient {
  return {
    async listModels() {

...
import {
    AdminLlmRoleSwitchDialogComponent,
    AdminLlmRoleSwitchDialogResult,
    LlmInstanceRole,
} from './admin-llm-role-switch-dialog.component';
...
export class AdminSpeechAssistantComponent implements OnInit {
    ...
    async saveInstanceConfig(): Promise<void> {
        ...
        try {
            ...
            this.frontendLogger.info( 'AdminSpeech', 'Saved instance-specific config', {
                instance: this.activeInstance?._id,
                model: this.activeInstance?.model,
    ...
    private getInstanceId(inst: LlmInstance | null | undefined): string | null {
        if ( !inst ) return null;
    ...
    private getRoleHolder(role: 'primary' | 'secondary', excludeId?: string | null): LlmInstance | null {
        const hit = (this.llmInstances || []).find( i => {
            const id = this.getInstanceId( i );
            if ( !id ) return false;
            if ( excludeId && id === excludeId ) return false;
            return this.normalizeRole( i.role ) === role;
        } );
        return hit || null;
    }
    ...
    async selectInstance(instance: LlmInstance | null): Promise<void> {
        ...
        console.log( '[AdminSpeech] 📋 Current config BEFORE select:', {
            url: this.config.url,
        ...
        try {
            ...
            this.frontendLogger.info( 'AdminSpeech', 'selectInstance completed', {
            ...
            this.frontendLogger.error( 'AdminSpeech', 'Failed to load instance config', {
                instanceId: instance._id,
                error
            } );
            this.snackBar.open( 'Fehler beim Laden der Konfiguration', 'OK', { duration: 3000 } );
    ...
    async saveSystemPrompt(): Promise<void> {
        ...
        try {
            ...
            this.frontendLogger.info( 'AdminSpeech', 'Saved system prompt', {
                instance: this.activeInstance?._id,
                length: this.systemPrompt.length
            } );
        } catch( error ) {
            console.error( 'Failed to save system prompt:', error );
            this.frontendLogger.error( 'AdminSpeech', 'Failed to save system prompt', {
                instance: this.activeInstance?._id,
                error
            } );
            this.snackBar.open( 'Fehler beim Speichern', 'OK', { duration: 3000 } );
    ...
    getLmStudioBaseUrl(url: unknown): string {
        ...
        try {
            ...
            // naive fallback
            return raw.replace( /\/v1\/chat\/completions$/i, '' ).replace( /\/chat\/completions$/i, '' ).replace( /\/+$/g, '' );
        }
    }
    ...
    getInstanceStatusLabel(inst: LlmInstance | null | undefined): string {
        ...
        if ( inst.role === 'secondary' || inst.role === 'primary' ) return 'Aktiv';
        if ( inst.role === 'other' ) return 'Inaktiv';
    ...
    protected changeLlmRole(instance: LlmInstance): void {
        // Der Click kommt aktuell vom Badge. Kein Propagation-Stop nötig.
        this.openRoleSwitchDialog( instance ).then();
    }

    private async openRoleSwitchDialog(instance: LlmInstance): Promise<void> {
        const id = this.getInstanceId( instance );
        if ( !id ) {
            this.snackBar.open( 'Instanz hat keine ID – Rolle kann nicht geändert werden', 'OK', { duration: 3500 } );
            return;
        }

        const currentRole = this.normalizeRole( instance.role ) as LlmInstanceRole;

        const primaryHolder = this.getRoleHolder( 'primary', id );
        const secondaryHolder = this.getRoleHolder( 'secondary', id );

        const dialogRef = this.dialog.open( AdminLlmRoleSwitchDialogComponent, {
            width: '560px',
            disableClose: false,
            data: {
                model: instance.model,
                currentRole,
                conflicts: {
                    primary: primaryHolder
                        ? {
                            role: 'primary',
                            otherInstanceId: this.getInstanceId( primaryHolder )!,
                            otherInstanceLabel: `${ primaryHolder.model } (${ this.getInstanceId( primaryHolder )! })`,
                        }
                        : undefined,
                    secondary: secondaryHolder
                        ? {
                            role: 'secondary',
                            otherInstanceId: this.getInstanceId( secondaryHolder )!,
                            otherInstanceLabel: `${ secondaryHolder.model } (${ this.getInstanceId( secondaryHolder )! })`,
                        }
                        : undefined,
                },
            },
        ...
        const res = (await dialogRef.afterClosed().toPromise()) as AdminLlmRoleSwitchDialogResult | null | undefined;
        if ( !res?.newRole ) return;
        ...
        try {
            // 1) Zielrolle setzen
            await lastValueFrom( this.llmService.setRole( id, newRole ) );

            // 2) Falls primary/secondary: Exklusivität erzwingen, aber nur wenn User Konflikt bestätigt hat.
            if ( newRole === 'primary' || newRole === 'secondary' ) {
                const conflict = newRole === 'primary' ? primaryHolder : secondaryHolder;

                if ( conflict ) {
                    // Dialog erzwingt aktuell nur „replace“ als bestätigbare Option
                    const conflictId = this.getInstanceId( conflict );
                    if ( conflictId ) {
                        await lastValueFrom( this.llmService.setRole( conflictId, 'other' ) );
                    }
                }

                // Safety net: wenn Daten drifted (mehr als ein Primary/Secondary), demote alle anderen.
                await this.enforceExclusiveRole( id, newRole );
            }

            this.snackBar.open( `Rolle geändert: ${ instance.model } → ${ newRole }`, 'OK', { duration: 3500 } );
        } catch( error ) {
            console.error( 'Failed to change LLM role:', error );
            this.snackBar.open( 'Rolle ändern fehlgeschlagen', 'OK', { duration: 3500 } );
        } finally {
            try {
                await this.loadLlmInstances();
            } catch {
    ...
    protected getRoleBadge(instance: LlmInstance): any {
        if (instance.role === 'other') return '';
